...
export const useGameStore = create<State & Actions>()((set, get) => ({
  ...
  addAlreadyFoundWord: (word: string) => {
    set((state) => ({ alreadyFoundWords: [...state.alreadyFoundWords, word] }));
  },
  ...
  fetchLevelWords: async () => {
    ...
    while (attempts < 5) {
      set({
        ...
        grid: {},
        revealIndex: 0,
        levelColor: `hsl(${360 * Math.random()}, ${25 + 70 * Math.random()}%, ${65 + 20 * Math.random()}%)`,
      });
      attempts++;
      try {
  ...
  revealLetter: () => {
    const { grid, revealIndex, levelWords, input, handleInputClick, checkWin } =
      get();
    const newGrid = { ...grid };
